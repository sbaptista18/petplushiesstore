...
const Checkout = () => {
  ...
  const fetchCartId = async (cartId) => {
    const options = {
      method: "GET",
      url: `//127.0.0.1:8000/temp_carts/id?id=${cartId}`,
    };
  ...
  const fetchProducts = (data) => {
    const promises = data.map((cartItem) => {
      const options = {
        method: "GET",
        url: `//127.0.0.1:8000/products/id?id=${cartItem.product_id}`,
      };
  ...
  const fetchShippingZonesDetails = async (area) => {
    const options = {
      method: "GET",
      url: `//127.0.0.1:8000/shipping?area=${area}`,
    };
      ...
      .then(() => {
        ...
        axios
          ...
          .then(function (response) {
            const options = {
              method: "DELETE",
              url: `//127.0.0.1:8000/temp_cart_delete_on_order?cartId=${cartId}`,
            };

...
const Checkout = () => {
  ...
  const fetchCartProducts = async (cartId) => {
    const options = {
      method: "GET",
      url: `/temp_cart_products_id?cartId=${cartId}`,
    };
  ...
  const fetchProducts = (data) => {
    const promises = data.map((cartItem) => {
      const options = {
        method: "GET",
        url: `/products/id?id=${cartItem.product_id}`,
      };
      ...
      .then(() => {
        ...
        axios
          ...
          .then(function (response) {
            const options = {
              method: "DELETE",
              url: `/temp_cart_delete_on_order?cartId=${cartId}`,
            };

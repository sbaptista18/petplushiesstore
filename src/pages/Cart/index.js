...
const Cart = () => {
  ...
  const fetchCartProducts = async (cartId) => {
    const url = `http://0.0.0.0:8000/temp_cart_products_id?cartId=${cartId}`;
  ...
  const fetchProducts = async (data) => {
    const promises = data.map((cartItem) => {
      const url = `http://0.0.0.0:8000/products/id?id=${cartItem.product_id}`;
  ...
  const updateCartProducts = (cartId, product_id, qty, product_price) => {
    ...
    const url = `http://0.0.0.0:8000/temp_cart_products_id`;
  ...
  const deleteCartProducts = (cartId, product_id) => {
    const options = {
      method: "DELETE",
      url: `http://0.0.0.0:8000/temp_cart_products_delete?cartId=${cartId}&prodId=${product_id}`,
    };
